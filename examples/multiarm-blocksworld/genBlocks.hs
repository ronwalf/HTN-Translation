...
makeStateWithBlocks blocks = do
    ...
    where
        tableBlocks [] _ = error "tableBlocks: empty list"
        tableBlocks (h:restFree) rootedTowers = do
            return $ (True, snd h) : rootedTowers ++ restFree
        stackBlocks [] _ = error "stackBlocks: empty list"
...
makeBlockProblem ::
    ...
    -> [[String]]
    -> PDDLProblem
makeBlockProblem pname armCount blockNames stacks gstacks =
    let
        arms = ["arm" ++ show n | n <- [1..armCount]]
        consts = 
            [ eTyped (eConst a :: Expr Const) ["ARM"] | a <- arms] ++
            [ eTyped (eConst b :: Expr Const) ["BLOCK"] | b <- blockNames]
    ...
    setConstants consts $
    setInitial (
        [eAtomic "hand-empty" ([eConst a] :: [ConstTermExpr])
        | a <- arms]
...
makeBlocksFiles :: String -> Int -> Int -> IO ()
makeBlocksFiles fname arms blockCount = do
    let blocks = ['b': show n | n <- [1..blockCount]]
    initState <- genBlocks blocks
    goalState <- genBlocks blocks
    let prob = makeBlockProblem fname arms blocks initState goalState
    
    writeFile (fname ++ ".pddl") (show $ pddlDoc prob)
    -- writeFile (fname ++ ".htn.pddl") (show hprob)

main :: IO ()
main = do
    (fname, [arms, blockStart, blockEnd, blockStep]) <- getArgs >>= \x -> do
        when (length x /= 5) $ fail "Usage: $./genBlocks file-prefix armcount min-blocks max-blocks step"
        counts <- flip mapM (tail x) $ \arg ->
            case reads arg of
                [(n :: Int, "")] -> return n
                _ -> fail $ "Cannot parse numeric argument '" ++ show arg ++"'"
        return (head x, counts)
    let blockCounts = toCount blockStart blockEnd blockStep
    let fmtStr = fname ++ "_%0" ++ (show $ length $ show blockEnd) ++ "d"
    sequence_ [
        makeBlocksFiles (printf fmtStr blocks) arms blocks
        | blocks <- blockCounts ]
        
    where
        toCount :: Int -> Int -> Int -> [Int]
        toCount start end step = 
            takeWhile (<= end) $ 
            iterate (+ step) start
